...
# Where install-fabric.sh typically clones fabric-samples (see `make install`).
...
help:
	...
	@echo "  make install   mkdir workdir, curl script, install -f $(FABRIC_VERSION) -c $(FABRIC_CA_VERSION)"
	...
	@echo "  make setup   network + createChannel + addOrg3 + deploy delivery chaincode (3-org)"
	...
	@echo ""
	@echo "Default UI/API ports: 8081–8083 (API), 8091–8093 (web) — see docker-compose.yml"
	@echo ""

_up-precheck: fabric-samples-precheck
	@FABRIC_SAMPLES_ROOT="$(FABRIC_SAMPLES_ROOT)" bash "$(ROOT)/scripts/check-fabric-msp.sh"
...
fabric-samples-precheck:
	@test -f "$(FABRIC_TEST_NETWORK)/network.sh" || ( \
		...
		echo "  FABRIC_SAMPLES_ROOT=$(FABRIC_INSTALL_WORKDIR)/fabric-samples"; \
		echo "If you installed with: make install FABRIC_GITHUB_USER=X, run:"; \
		echo "  make setup FABRIC_GITHUB_USER=X"; \
...
# Start peers/orderer/CA and create channel mychannel (Docker on host). Fails if channel already exists and peers already joined — use fabric-network-up + fabric-deploy-chaincode, or make clean for a full reset.
fabric-network: fabric-samples-precheck
	cd "$(FABRIC_TEST_NETWORK)" && ./network.sh up createChannel

# Join Org3 to mychannel (run after fabric-network). Safe to re-run if Org3 is already on the channel (script may error — use make clean for a clean slate).
...
clean: fabric-samples-precheck
	...
	@echo ""
	@echo "Ledger cleared. Recreate: make setup"
